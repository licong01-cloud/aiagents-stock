"""
季报数据获取模块
优先使用Tushare获取个股最近8期季度财务报告，失败时使用akshare作为备用数据源
"""
    ...
    def _get_income_statement(self, symbol):
        ...
        try:
            ...
            if self.data_source_manager.tushare_available:
                try:
                    print(f"   [Tushare] 正在获取利润表数据...")
                    ts_code = self.data_source_manager._convert_to_ts_code(symbol)
                    df = self.data_source_manager.tushare_api.income(ts_code=ts_code, period='')
                    
                    if df is not None and not df.empty:
                        # 获取最近8期
                        df = df.head(self.periods)
                        print(f"   [Tushare] ✅ 成功获取 {len(df)} 期利润表数据")
            ...
            # Tushare失败，使用akshare
            print(f"   [Akshare] 正在获取利润表数据（备用数据源）...")
            with network_optimizer.apply():
                df = ak.stock_financial_report_sina(stock=symbol, symbol="利润表")
            
            if df is None or df.empty:
                print(f"   [Akshare] 未找到利润表数据")
                return None
            ...
            print(f"   [Akshare] ✅ 成功获取 {len(data_list)} 期利润表数据")
            return {
                "data": data_list,
                "periods": len(data_list),
                "columns": df.columns.tolist(),
