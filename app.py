...
def main():
        ...
        with st.expander("🎯 选股板块", expanded=True):
            ...
            if st.button("⭐ 自选股票池", width='stretch', key="nav_watchlist", help="管理自选股票池"):
                ...
                for key in ['show_history', 'show_monitor', 'show_config', 'show_sector_strategy',
                           'show_longhubang', 'show_portfolio', 'show_local_data', 'show_smart_monitor', 'show_main_force', 'show_hotboard']:
                    if key in st.session_state:
                        del st.session_state[key]
    ...
    with col1:
        ...
        enable_research = st.checkbox("📑 机构研报分析师", value=False,
                                     help="梳理机构研报观点、评级与目标价，聚合一致与分歧")
    ...
    with col2:
        ...
        enable_announcement = st.checkbox("📢 公告分析师", value=False,
                                        help="解析重大公告事件、判断利多利空与时间敏感度")
    ...
    with col3:
        ...
        enable_chip = st.checkbox("🎯 筹码分析师", value=False,
                                 help="分析筹码集中度、成本区间与主力控盘（仅A股）")
    ...
    if 'enable_research' in locals() and enable_research:
        selected_analysts.append("机构研报分析师")
    if 'enable_announcement' in locals() and enable_announcement:
        selected_analysts.append("公告分析师")
    if 'enable_chip' in locals() and enable_chip:
        selected_analysts.append("筹码分析师")
    ...
    st.session_state.enable_announcement = enable_announcement
    st.session_state.enable_chip = enable_chip
    ...
    if analyze_button and stock_input:
        ...
        # 校验市场支持范围（仅A股/港股）
        if analysis_mode == "单个分析":
            if not is_supported_symbol(stock_input):
                st.error("❌ 暂不支持美股（或非法代码）。仅支持A股与港股")
                return
        ...
        if analysis_mode == "单个分析":
            ...
            unsupported = [s for s in stock_list if not is_supported_symbol(s)]
            if unsupported:
                st.error(f"❌ 以下代码不支持（仅支持A股/港股）：{', '.join(unsupported)}")
                return
...
def get_stock_data(symbol, period, analysis_date=None):
    ...
    if not isinstance(stock_data, pd.DataFrame):
        debug_logger.error("get_stock_data返回类型错误",
                          ...
                          symbol=symbol,
                          period=period,
                          data_preview=str(stock_data)[:200])
        # 尝试返回None而不是继续处理
        return stock_info, None, None
    
    # 检查DataFrame是否为空
    if stock_data.empty:
        debug_logger.warning("get_stock_data返回空DataFrame", symbol=symbol, period=period)
        return stock_info, None, None

    # 计算技术指标
    try:
        debug_logger.debug("开始计算技术指标", symbol=symbol, rows=len(stock_data))
        stock_data_with_indicators = unified_fetcher.stock_data_fetcher.calculate_technical_indicators(stock_data)
        indicators = unified_fetcher.stock_data_fetcher.get_latest_indicators(stock_data_with_indicators)
        
        debug_logger.info("get_stock_data完成",
                         symbol=symbol,
                         rows=len(stock_data_with_indicators),
                         indicators_count=len(indicators) if isinstance(indicators, dict) else 0)
        
        return stock_info, stock_data_with_indicators, indicators
    except Exception as e:
        debug_logger.error("计算技术指标失败",
                          error=e,
                          symbol=symbol,
                          stock_data_type=type(stock_data).__name__,
                          stock_data_shape=getattr(stock_data, 'shape', 'N/A'))
        # 即使指标计算失败，也返回原始数据
...
def analyze_single_stock_for_batch(symbol, period, enabled_analysts_config=None, selected_model='deepseek-chat', analysis_date=None):
    ...
    try:
        ...
        # 6.5 新增三位分析师数据（A股优先，通过统一数据访问接口）
        research_data = None
        announcement_data = None
        chip_data = None
        
        enable_research = enabled_analysts_config.get('research', False)
        enable_announcement = enabled_analysts_config.get('announcement', False)
        enable_chip = enabled_analysts_config.get('chip', False)
        
        if unified_fetcher._is_chinese_stock(symbol):
            try:
                # 获取机构研报数据
                if enable_research:
                ...
                debug_logger.error("批量分析-获取新分析师数据失败",
                                 error=e,
                                 symbol=symbol)
        ...
        agents_results = agents.run_multi_agent_analysis(
            ...
            research_data, announcement_data, chip_data,
            enabled_analysts=enabled_analysts_config
        )
...
def run_batch_analysis(stock_list, period, batch_mode="顺序分析"):
    ...
    enabled_analysts_config = {
        ...
        'research': st.session_state.get('enable_research', False),
        'announcement': st.session_state.get('enable_announcement', False),
        'chip': st.session_state.get('enable_chip', False)
    }
    ...
    try:
        ...
        if enable_fundamental and unified_fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                quarterly_data = None
        elif enable_fundamental and not unified_fetcher._is_chinese_stock(symbol):
            st.info("ℹ️ 港股暂不支持季报数据")
        ...
        if enable_fund_flow and unified_fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                fund_flow_data = None
        elif enable_fund_flow and not unified_fetcher._is_chinese_stock(symbol):
            st.info("ℹ️ 港股暂不支持资金流向数据")
        ...
        if enable_sentiment and unified_fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                sentiment_data = None
        elif enable_sentiment and not unified_fetcher._is_chinese_stock(symbol):
            st.info("ℹ️ 港股暂不支持市场情绪数据（ARBR等指标）")
        ...
        if enable_risk and unified_fetcher._is_chinese_stock(symbol):
            ...
            try:
                ...
                risk_data = None
        elif enable_risk and not unified_fetcher._is_chinese_stock(symbol):
            st.info("ℹ️ 港股暂不支持风险数据（限售解禁、大股东减持等）")
        ...
        enable_research = st.session_state.get('enable_research', False)
        enable_announcement = st.session_state.get('enable_announcement', False)
        enable_chip = st.session_state.get('enable_chip', False)
            ...
            if enable_research:
                ...
                try:
                    ...
                    if research_data and research_data.get('data_success'):
                        ...
                        st.warning("⚠️ 未能获取机构研报数据")
                except Exception as e:
                    debug_logger.error("获取机构研报数据失败", error=e, symbol=symbol)
                    st.warning(f"⚠️ 获取机构研报数据时出错: {str(e)}")
                    research_data = None
            ...
            if enable_announcement:
                ...
                try:
                    ...
                    if announcement_data and announcement_data.get('data_success'):
                        ...
                        st.warning("⚠️ 未能获取公告数据")
                except Exception as e:
                    debug_logger.error("获取公告数据失败", error=e, symbol=symbol)
                    st.warning(f"⚠️ 获取公告数据时出错: {str(e)}")
                    announcement_data = None
                progress_bar.progress(53)
            
            # 获取筹码数据
            if enable_chip:
                status_text.text("🎯 正在获取筹码数据...")
                try:
                    # 传递当前价格用于筹码分析
                    current_price = stock_info.get('current_price') if stock_info else None
                    chip_data = unified_fetcher.get_chip_distribution_data(symbol, current_price=current_price, analysis_date=analysis_date)
                    if chip_data and chip_data.get('data_success'):
                        ...
                        st.warning("⚠️ 未能获取筹码分布数据")
                except Exception as e:
                    debug_logger.error("获取筹码数据失败", error=e, symbol=symbol)
                    st.warning(f"⚠️ 获取筹码数据时出错: {str(e)}")
                    chip_data = None
                progress_bar.progress(54)
        else:
            # 非A股提示
            if enable_research or enable_announcement or enable_chip:
                st.info("ℹ️ 研报、公告、筹码数据仅支持A股")
        ...
        debug_logger.error(f"股票分析过程出现异常", 
                          error=e, 
                          symbol=symbol, 
                          period=period)
...
def display_stock_chart(stock_data, stock_info):
    ...
    if missing_columns:
        ...
        st.write("可用列:", list(stock_data.columns))
        return
    
    # 检查数据是否为空
    if stock_data.empty:
        debug_logger.warning("stock_data为空DataFrame", stock_info=stock_info)
        st.warning("⚠️ 股票数据为空，无法显示图表")
        return
    
    debug_logger.debug("开始绘制股票图表",
                      rows=len(stock_data),
...
def display_agents_analysis(agents_results):
    ...
    # 显示性能统计（如果有）
    if '_performance' in agents_results:
        perf = agents_results['_performance']
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric("⏱️ 总用时", f"{perf['total_time']:.2f}秒")
        with col2:
            st.metric("⚡ 并行效率", f"节省 {perf['parallel_efficiency']:.2f}秒")
        with col3:
            st.metric("📊 分析师数量", f"{perf['analyst_count']}位")
        with col4:
            avg_time = sum(perf['analyst_times'].values()) / len(perf['analyst_times']) if perf['analyst_times'] else 0
            st.metric("📈 平均用时", f"{avg_time:.2f}秒")
        
        # 显示详细用时图表
        with st.expander("📊 查看各分析师用时详情", expanded=False):
            if perf['analyst_times']:
                # 获取分析师名称映射
                analyst_names = {}
                for key, result in agents_results.items():
                    if key != '_performance' and isinstance(result, dict):
                        analyst_names[key] = result.get('agent_name', key)
                
                # 准备图表数据
                timing_data = []
                for key, elapsed_time in perf['analyst_times'].items():
                    timing_data.append({
                        '分析师': analyst_names.get(key, key),
                        '用时(秒)': round(elapsed_time, 2)
                    })
                
                # 按用时排序
                timing_data = sorted(timing_data, key=lambda x: x['用时(秒)'], reverse=True)
                
                # 显示表格
                st.dataframe(timing_data, width='stretch', hide_index=True)
    ...
    for i, tab in enumerate(tabs):
        with tab:
            ...
            # 分析师信息（包含用时）
            elapsed_time_str = ""
            if 'elapsed_time' in agent_result:
                elapsed_time_str = f"<p><strong>⏱️ 分析用时：</strong>{agent_result['elapsed_time']:.2f}秒</p>"
            ...
            # 公告分析师：显示公告标题列表和下载链接
            if agent_name == '公告分析师' and agent_result.get('announcement_data'):
                announcement_data = agent_result.get('announcement_data')
                if announcement_data and announcement_data.get('data_success'):
                    st.markdown("---")
                    st.markdown("**📢 公告列表（点击标题查看详情）:**")
            ...
            if agent_name == '机构研报分析师' and agent_result.get('research_data'):
                ...
                if research_data and research_data.get('data_success'):
                    st.markdown("---")
                    st.markdown("**📑 研报列表（点击标题查看详情）:**")
                    # 尝试多种可能的字段名
                    reports = research_data.get('research_reports', []) or research_data.get('reports', []) or research_data.get('items', [])
                    if reports:
                        for idx, report in enumerate(reports, 1):
                            date = report.get('日期', 'N/A')
                            title = report.get('研报标题', 'N/A')
                            org = report.get('机构名称', 'N/A')
                            rating = report.get('评级', 'N/A')
                            target_price = report.get('目标价', 'N/A')
                            
                            col1, col2 = st.columns([4, 1])
                            with col1:
                                st.markdown(f"**{idx}. [{date}] {title}**")
                                st.caption(f"机构: {org} | 评级: {rating} | 目标价: {target_price}")
                            with col2:
                                # 研报下载链接（如果有）
                                report_link = report.get('研报链接', '')
                                if report_link and report_link != 'N/A':
                                    st.markdown(f"[📥 下载]({report_link})")
                                else:
                                    st.markdown("📄 无链接")
            
            # 新闻分析师：显示新闻标题列表和下载链接
            if agent_name == '新闻分析师' and agent_result.get('news_data'):
                news_data = agent_result.get('news_data')
                if news_data and news_data.get('data_success'):
                    st.markdown("---")
                    st.markdown("**📰 新闻列表（点击标题查看详情）:**")
                    news_items = news_data.get('news_items', []) or news_data.get('items', [])
                    if news_items:
                        for idx, news in enumerate(news_items, 1):
                            date = news.get('date', news.get('日期', 'N/A'))
                            title = news.get('title', news.get('标题', 'N/A'))
                            source = news.get('source', news.get('来源', 'N/A'))
                            ...
                            with col2:
                                # 新闻链接（如果有）
                                news_link = news.get('link', news.get('链接', ''))
                                if news_link and news_link != 'N/A':
                                    st.markdown(f"[🔗 查看]({news_link})")
                                else:
                                    st.markdown("📄 无链接")
...
def show_example_interface():
    ...
    st.markdown("""
    ### 🌏 市场支持说明
    - **A股**：完整支持（技术分析、财务数据、资金流向、市场情绪、新闻数据qstock、季报）
    - **港股**：部分支持（技术分析、部分财务指标）⭐️ 
        ...
        with st.form(key=f"monitor_form_{record['id']}"):
            ...
            with col2:
                ...
                new_rating = st.selectbox("投资评级", ["买入", "持有", "卖出"],
                                         index=safe_index(["买入", "持有", "卖出"], rating, default=0))
...
def display_config_manager():
    ...
    with tab2:
        ...
        # 启用代理
        use_proxy_val = st.checkbox(
            "启用代理",
            value=st.session_state.temp_config.get("USE_PROXY", "false") == "true",
            help="开启后优先使用代理（先静态、后动态代理池）",
            key="input_use_proxy"
        )
        st.session_state.temp_config["USE_PROXY"] = "true" if use_proxy_val else "false"

        # 动态代理池
        proxypool_enabled = st.checkbox(
            "启用动态代理池",
            value=st.session_state.temp_config.get("PROXYPOOL_ENABLED", "false") == "true",
            help="从代理池API定期拉取可用代理并轮询使用",
            key="input_proxypool_enabled"
        )
        st.session_state.temp_config["PROXYPOOL_ENABLED"] = "true" if proxypool_enabled else "false"

        col_p1, col_p2 = st.columns(2)
        with col_p1:
            base_url = st.text_input(
                "代理池Base URL",
                value=st.session_state.temp_config.get("PROXYPOOL_BASE_URL", ""),
                placeholder="如: https://proxy-pool.example.com/get",
                disabled=not proxypool_enabled,
                key="input_proxypool_base_url"
            )
            st.session_state.temp_config["PROXYPOOL_BASE_URL"] = base_url

        with col_p2:
            refresh_min = st.text_input(
                "刷新间隔(分钟)",
                value=st.session_state.temp_config.get("PROXY_REFRESH_INTERVAL_MIN", "10"),
                disabled=not proxypool_enabled,
                key="input_proxy_refresh_interval"
            )
            st.session_state.temp_config["PROXY_REFRESH_INTERVAL_MIN"] = refresh_min

        auth_type = st.selectbox(
            "鉴权方式",
            options=["token", "basic", "urlparam"],
            index=safe_index(["token", "basic", "urlparam"], st.session_state.temp_config.get("PROXYPOOL_AUTH_TYPE", "token"), default=0),
            disabled=not proxypool_enabled,
            key="input_proxypool_auth_type"
        )
        st.session_state.temp_config["PROXYPOOL_AUTH_TYPE"] = auth_type

        if auth_type == "token" or auth_type == "urlparam":
            token_val = st.text_input(
                "代理池Token",
                value=st.session_state.temp_config.get("PROXYPOOL_TOKEN", ""),
                type="password",
                disabled=not proxypool_enabled,
                key="input_proxypool_token"
            )
            st.session_state.temp_config["PROXYPOOL_TOKEN"] = token_val
        elif auth_type == "basic":
            col_b1, col_b2 = st.columns(2)
            with col_b1:
                username_val = st.text_input(
                    "用户名",
                    value=st.session_state.temp_config.get("PROXYPOOL_USERNAME", ""),
                    disabled=not proxypool_enabled,
                    key="input_proxypool_username"
                )
                st.session_state.temp_config["PROXYPOOL_USERNAME"] = username_val
            with col_b2:
                password_val = st.text_input(
                    "密码",
                    value=st.session_state.temp_config.get("PROXYPOOL_PASSWORD", ""),
                    type="password",
                    disabled=not proxypool_enabled,
                    key="input_proxypool_password"
                )
                st.session_state.temp_config["PROXYPOOL_PASSWORD"] = password_val

        # 文档导入（手工粘贴）
        with st.expander("📥 从文档导入（手工粘贴）"):
            st.caption("请从 newdoc 的《网络优化模块可移植指南》复制代理池Base URL与鉴权信息粘贴到下方：")
            imp_base = st.text_input("Base URL", value="", placeholder="https://...")
            imp_token = st.text_input("Token（若适用）", value="", type="password")
            imp_user = st.text_input("用户名（basic鉴权）", value="")
            imp_pass = st.text_input("密码（basic鉴权）", value="", type="password")
            if st.button("应用导入", key="btn_apply_import_proxy"):
                if imp_base:
                    st.session_state.temp_config["PROXYPOOL_BASE_URL"] = imp_base
                if imp_token:
                    st.session_state.temp_config["PROXYPOOL_TOKEN"] = imp_token
                if imp_user:
                    st.session_state.temp_config["PROXYPOOL_USERNAME"] = imp_user
                if imp_pass:
                    st.session_state.temp_config["PROXYPOOL_PASSWORD"] = imp_pass
                st.success("✅ 已应用导入信息到临时配置（保存配置后生效）")

        # 测试代理池
        if proxypool_enabled and base_url:
            if st.button("🧪 测试动态代理", key="btn_test_proxypool"):
                with st.spinner("正在测试动态代理获取与连通..."):
                    # 临时覆盖环境变量
                    keys = [
                        "USE_PROXY","PROXYPOOL_ENABLED","PROXYPOOL_BASE_URL","PROXYPOOL_AUTH_TYPE",
                        "PROXYPOOL_TOKEN","PROXYPOOL_USERNAME","PROXYPOOL_PASSWORD","PROXY_REFRESH_INTERVAL_MIN"
                    ]
                    backup = {k: os.getenv(k) for k in keys}
                    for k in keys:
                        os.environ[k] = st.session_state.temp_config.get(k, "")
                    try:
                        temp_opt = NetworkOptimizer()
                        proxy = temp_opt.get_dynamic_proxy()
                        if proxy and temp_opt.test_proxy_fast({"proxy": proxy}):
                            st.success(f"✅ 成功获取并连通动态代理: {proxy}")
                        else:
                            st.warning("⚠️ 无法获取或连通动态代理，请检查Base URL与鉴权配置")
                    except Exception as e:
                        st.error(f"❌ 测试失败: {e}")
                    finally:
                        # 还原环境变量
        ...
        # 测试网络连通（按当前已保存配置状态）
        if st.button("🌐 测试网络连通", key="btn_test_network"):
            with st.spinner("正在测试网络连通..."):
                ok = network_optimizer.test_network_connection()
                if ok:
                    st.success("✅ 网络连通正常")
                else:
                    st.warning("⚠️ 网络不可达或较慢，请检查代理/网络设置")
