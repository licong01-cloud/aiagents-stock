...
class ConfigManager:
    ...
    def write_env(self, config: Dict[str, str]) -> bool:
        ...
        try:
            ...
            lines.append(f'PROXYPOOL_ENABLED="{config.get("PROXYPOOL_ENABLED", "false")}"')
            lines.append(f'PROXYPOOL_BASE_URL="{config.get("PROXYPOOL_BASE_URL", "")}"')
            lines.append(f'PROXYPOOL_AUTH_TYPE="{config.get("PROXYPOOL_AUTH_TYPE", "token")}"')
            lines.append(f'PROXYPOOL_TOKEN="{config.get("PROXYPOOL_TOKEN", "")}"')
            lines.append(f'PROXYPOOL_USERNAME="{config.get("PROXYPOOL_USERNAME", "")}"')
            lines.append(f'PROXYPOOL_PASSWORD="{config.get("PROXYPOOL_PASSWORD", "")}"')
            lines.append(f'PROXY_REFRESH_INTERVAL_MIN="{config.get("PROXY_REFRESH_INTERVAL_MIN", "10")}"')
